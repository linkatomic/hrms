// Check-in screen — hold-to-clock with live timer and break tracking
const { useEffect: useEffect_ci, useRef: useRef_ci, useState: useState_ci } = React;

const Checkin = ({ data, clock, setClock }) => {
  const holdRef = useRef_ci(null);
  const fillRef = useRef_ci(null);
  const [progress, setProgress] = useState_ci(0);
  const rafRef = useRef_ci(null);
  const startedRef = useRef_ci(null);
  ...
  const statusLabel = {
    out: "Tap to start", in: "Clocked in", break: "On break", done: "Day complete",
  }[status];

  return (
    <div data-screen-label="02 Check-in">
      ...
      <div className="brut-card" style={{ padding: 22 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(14, 1fr)", gap: 5, alignItems: "end", height: 140 }}>
          {[
            ...
            const max = (b.h / 10) * 140;
            const color = b.k === "in" ? "var(--accent)" : b.k === "wfh" ? "var(--info)" : b.k === "absent" ? "var(--bad)" : "var(--bg-elev-2)";
            return (
              <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                <div style={{ flex: 1, width: "100%", display: "flex", alignItems: "flex-end" }}>
                  <div style={{ width: "100%", height: Math.max(b.h ? 10 : 4, max), background: color, border: "1.5px solid var(--border-strong)" }}></div>
                </div>
